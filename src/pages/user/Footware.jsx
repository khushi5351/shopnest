...
function Footware() {
    ...
    async function fetchMen() {
        try {
            const res = await axios.get(`${URL}?category=Footwear`);
            setFootware(res.data);
        } catch (error) {
            console.error("Failed to fetch Footware products:", error);
        }
    }
    ...
    const totalPages = Math.ceil(Footware.length / productsPerPage);
    const indexOfLast = currentPage * productsPerPage;
    const indexOfFirst = indexOfLast - productsPerPage;
    // const currentProducts = Footware.slice(indexOfFirst, indexOfLast);
