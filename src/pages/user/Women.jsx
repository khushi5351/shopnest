...
function Women() {
    ...
    async function fetchWomen() {
        try {
            const res = await axios.get(`${URL}?category=Women's Clothing`);
            setWomen(res.data);
        } catch (error) {
    ...
    const totalPages = Math.ceil(women.length / productsPerPage);
    const indexOfLast = currentPage * productsPerPage;
    const indexOfFirst = indexOfLast - productsPerPage;
    const currentProducts = women.slice(indexOfFirst, indexOfLast);
